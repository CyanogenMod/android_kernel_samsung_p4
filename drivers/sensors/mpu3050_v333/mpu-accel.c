					  ...
					  unsigned char *buffer)
{
	...
	if (NULL != mldl_cfg->accel && NULL != mldl_cfg->accel->read)
		result =
		    mldl_cfg->accel->read(data->accel_handle, mldl_cfg->accel,
					  &mldl_cfg->pdata->accel, buffer);
	return result;
}

static int mpu_accel_get_data_from_mpu(struct mpuaccel_data *data,
				       unsigned char *buffer)
{
	int result = ML_SUCCESS;
	struct mldl_cfg *mldl_cfg = data->mldl_cfg;
...
static int mpu_accel_get_data(struct mpuaccel_data *data,
			      unsigned char *buffer, int *from_mpu)
{
	int res = ML_SUCCESS;
	struct mldl_cfg *mldl_cfg = data->mldl_cfg;
...
static ssize_t mpu_accel_enable_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	struct input_dev *input_data = to_input_dev(dev);

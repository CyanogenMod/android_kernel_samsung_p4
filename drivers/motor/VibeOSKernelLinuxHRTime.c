...
/*
** Kernel high-resolution software timer is used
** as an example but another type
...
"Either provide a non hr-timer implementation of VibeOSKernelLinuxTime.c"
"or re-compile your kernel with CONFIG_HIGH_RES_TIMERS=y"
#endif
...
static int VibeOSKernelProcessData(void *data)
{
	...
	for (i = 0; i < NUM_ACTUATORS; i++) {
		struct actuator_samples_buffer *pCurrentActuatorSample
			= &(g_SamplesBuffer[i]);
		...
		if (-1 == pCurrentActuatorSample->nIndexPlayingBuffer) {
			...
			if (VIBE_E_FAIL == ImmVibeSPI_ForceOut_SetSamples(
				pCurrentActuatorSample->actuatorSamples
				[(int)pCurrentActuatorSample->
				nIndexPlayingBuffer].nActuatorIndex,
				pCurrentActuatorSample->actuatorSamples
				[(int)pCurrentActuatorSample->
				...
				pCurrentActuatorSample->actuatorSamples
				[(int)pCurrentActuatorSample->
				nIndexPlayingBuffer].dataBuffer))
				/*
			...
			if (pCurrentActuatorSample->nIndexOutputValue
				...
				pCurrentActuatorSample->
					actuatorSamples
					[(int)pCurrentActuatorSample->
					nIndexPlayingBuffer].nBufferSize = 0;
...
static void VibeOSKernelLinuxStartTimer(void)
{
	...
	*/
	...
	if (res != 0)
		DbgOut((KERN_INFO "%s: down_interruptible"

...
static int __init spi_tegra_probe(struct platform_device *pdev)
{
	...
	tspi->rx_dma = tegra_dma_allocate_channel(TEGRA_DMA_MODE_ONESHOT,"spi_rx_%d", pdev->id);
	if (!tspi->rx_dma) {
	...
	tspi->tx_dma = tegra_dma_allocate_channel(TEGRA_DMA_MODE_ONESHOT, "spi_tx_%d", pdev->id);

...
void p3_uart_path_init(void){
	...
	//gpio_set_value(GPIO_UART_SEL, 1); // Set UART path to AP
	//gpio_set_value(GPIO_UART_SEL2, 0); // Set UART_SEL2 always initiate 1
	//gpio_set_value(GPIO_UART_SEL, 0); // Set UART path to CP
...
static DEVICE_ATTR(uartsel, \
	S_IRUGO | S_IWUSR | S_IWGRP, \
	uart_sel_show, uart_sel_store);	/*EUR*/
...
static DEVICE_ATTR(usbsel, \
	S_IRUGO | S_IWUSR | S_IWGRP, \
		usb_sel_show, usb_sel_store);	/*EUR*/

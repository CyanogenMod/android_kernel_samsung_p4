...
#define DEBUG_LEVEL_FILE_NAME	"/dev/block/mmcblk0p5"
...
void kernel_sec_set_cp_ack(void)
{
	/* received_cp_ack = 1; */
}
...
void sec_set_cp_upload(void)
{
	...
	cp_dpram_mbx_BA = (volatile u16 *)(dpram_base + 0x3FFE);
	cp_dpram_mbx_AB = (volatile u16 *)(dpram_base + 0x3FFC);

	/*KERNEL_SEC_DUMP_AP_DEAD_INDICATOR_DPRAM*/
	send_mail = MODEM_FORCE_CRASH;
...
{
	dpram_base = ioremap_nocache(DPRAM_START_ADDRESS, DPRAM_SIZE);

	if (dpram_base == NULL)
		printk(KERN_ERR "%s : failed ioremap\n", __func__);
	else
		printk(KERN_ERR "%s : sec_cp_upload_init Done !!\n", __func__);
...
bool kernel_sec_set_debug_level(int level)
{
	...
	printk(KERN_NOTICE "The debug value is 0x%x !!\n", level);

...
int __init p4_charge_init(void)
{
...
static void p4_board_suspend(int lp_state, enum suspend_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_SUSPEND_BEFORE_CPU))
		tegra_console_uart_suspend();
}

static void p4_board_resume(int lp_state, enum resume_stage stg)
{
...
static struct tegra_suspend_platform_data p3_suspend_data = {
	...
	.corereq_high	= false,
	.sysclkreq_high	= true,
	.board_suspend = p4_board_suspend,
	.board_resume = p4_board_resume,
};
...
static char *p4_battery[] = {
	"battery",
};

static struct gpio_charger_platform_data p4_charger_pdata = {
	.name = "ac",
	.type = POWER_SUPPLY_TYPE_MAINS,
	.gpio = AC_PRESENT_GPIO,
	.gpio_active_low = 1,
	.supplied_to = p4_battery,
	.num_supplicants = ARRAY_SIZE(p4_battery),
};

static struct platform_device p4_charger_device = {
	.name = "gpio-charger",
	.dev = {
		.platform_data = &p4_charger_pdata,
	},
};

int __init p4_charger_init(void)
{
	tegra_gpio_enable(AC_PRESENT_GPIO);
	platform_device_register(&p4_charger_device);
return 0;
}
